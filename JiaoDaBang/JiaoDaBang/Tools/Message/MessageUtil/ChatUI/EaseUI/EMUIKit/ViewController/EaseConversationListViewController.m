...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //customer config...
    if ([[MessageUtil shareMessageManage] isOrderConversationId:model.conversation.conversationId]) {
        cell.forceShowBadge = YES;
    }
    else {
        cell.forceShowBadge = NO;
    }
...
- (void)tableViewDidTriggerHeaderRefresh
{
    ...
    dispatch_async(refreshQueue, ^{
        ...
        [weakself fetchConversationChatterInfo:weakself.dataArray];
    });
    
    
}

- (void)fetchConversationChatterInfo:(NSArray *)dataArray {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (EaseConversationModel *model in dataArray)
        {
            if (!model || ![model isKindOfClass:[EaseConversationModel class]] || !model.conversation.conversationId || model.conversation.conversationId.length <=0) {
                return ;
            }
            [[MessageUtil shareMessageManage] fetchChatterInfo:model.conversation.conversationId];
        }
    });
}
