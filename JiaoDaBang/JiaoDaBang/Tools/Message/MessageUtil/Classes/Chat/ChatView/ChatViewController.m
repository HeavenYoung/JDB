...
@interface ChatViewController ()<UIAlertViewDelegate, EaseMessageViewControllerDelegate, EaseMessageViewControllerDataSource,EMClientDelegate,MessageOrderTypeCellDelegate>
{
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.conversation.type == EMConversationTypeChat) {
        if ([self.conversation.conversationId length])
        {
            self.title = [MessageUserProfileUtil getNickNameWithUsername:self.conversation.conversationId];
        }
    }
}
...
- (void)setNavigationType:(NSString *)navigationType {
    ...
    //cancel/back
    if ([navigationType isEqualToString:@"cancel"] ) {
        // 1. 左按钮
...
//    UIBarButtonItem *backItem = [[UIBarButtonItem alloc] initWithCustomView:backButton];
    ...
    if (![self.navigationType isEqualToString:@"cancel"]) {
//        [self.navigationItem setLeftBarButtonItem:backItem];
        self.navigationItem.leftBarButtonItem = [UIBarButtonItem barButtonWithTitle:nil titleFont:[UIFont systemFontOfSize:16] titleColor:[UIColor whiteColor] imageName:@"back.png" target:self action:@selector(backAction)];
    }
   ...
   didLongPressRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (![object isKindOfClass:[NSString class]]) {
        UITableViewCell *msgCell = [self.tableView cellForRowAtIndexPath:indexPath];
//        EaseMessageCell *cell = (EaseMessageCell *)[self.tableView cellForRowAtIndexPath:indexPath];
        if ([msgCell isKindOfClass:[EaseMessageCell class]]) {
            EaseMessageCell *cell = (EaseMessageCell *)msgCell;
...
{
    if (messageModel.bodyType == EMMessageBodyTypeImage && [[messageModel.message.ext objectForKey:@"orderType"] isEqualToString:@"orderType"]) {

        MessageOrderTypeCell *msgCell = [[MessageOrderTypeCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil model:messageModel];
        if (msgCell) {
...
//    model.avatarImage = [UIImage imageNamed:@"EaseUIResource.bundle/user"];
    model.avatarImage = [UIImage imageNamed:@"userMT"];
//    UserProfileEntity *profileEntity = [[UserProfileManager sharedInstance] getUserProfileByUsername:model.nickname];
//    if (profileEntity) {
//        model.avatarURLPath = profileEntity.imageUrl;
//        model.nickname = profileEntity.nickname;
//    }
    NSString *chatterId = model.nickname;
    
    //根据是否是当前登录者发送的消息,根据userid获取头像、昵称
    if (model.isSender) {
        chatterId = [GlobalManager sharedManager].hxUserId;
    }
    else {
        chatterId = self.conversation.conversationId;
    }
    model.avatarURLPath = [MessageUserProfileUtil getavatarURLPathWithUsername:chatterId];
    model.nickname = [MessageUserProfileUtil getNickNameWithUsername:chatterId];
...
//    [[ChatDemoHelper shareHelper] setChatVC:nil];
    [[MessageUtil shareMessageManage] setChatVC:nil];
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:kMessageToOrderNotification object:nil];
}
