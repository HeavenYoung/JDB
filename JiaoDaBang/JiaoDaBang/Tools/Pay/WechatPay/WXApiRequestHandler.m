...
+ (BOOL)sendLinkURL:(NSString *)urlString
    ...
    if (![WXApi isWXAppInstalled]) {
        UIAlertView *alter = [[UIAlertView alloc]initWithTitle:@"提示" message:@"微信客户端没有安装，请先安装！" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alter show];
        return NO;
    }
...
+ (NSString *)jumpToBizPay {
    ...
    NSString *aString = [[NSString alloc] initWithData:response encoding:NSUTF8StringEncoding];
    if ( response != nil) {
...
+ (NSString *)WXPay:(NSDictionary *)orderParams {
    ...
    req.timeStamp = (UInt32)[[orderParams objectForKey:@"timestamp"] integerValue];
...
+ (NSString *)WXOrderPay:(WXOrderPayData *)orderInfo {
    ...
    req.timeStamp = (UInt32)[orderInfo.timestamp integerValue];
