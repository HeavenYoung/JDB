...
@synthesize isDimBackground = _isDimBackground;
...
- (void)showAlertHandle{
    ...
    vc.delegate = self;
    vc.alertView = self;
    vc.dimBackground = self.isDimBackground;
    self.vc = vc;
...
- (void)coverViewTouched{
    if (self.isDismissWhenTouchBackground) {
        ...
        if ([self.delegate respondsToSelector:@selector(didTouchBackground)]) {
            [self.delegate didTouchBackground];
        }
    }
}
