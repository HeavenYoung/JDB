...
- (UIView *) mkContentView {
    ...
    UIImage *gradientLine = [YCXMenuView gradientLine: (CGSize){maxItemWidth , 1}]; // - kMarginX * 4
    ...
    for (YCXMenuItem *menuItem in _menuItems) {
        ...
        if (menuItem.title.length) {
            ...
            titleLabel.textColor = menuItem.foreColor ? menuItem.foreColor : [UIColor colorWithHexString:@"6d6d6d"];
...
+ (UIImage *)gradientLine:(CGSize)size {
    const CGFloat locations[] = {0,1};
    const CGFloat R = 0.9, G = 0.9, B = 0.9;
    const CGFloat components[] = {
        R,G,B,1,
        R,G,B,1,
    };
...
//        [MBProgressHUD showError:@"该用户还么有设置更多！"];
        return;
    }
