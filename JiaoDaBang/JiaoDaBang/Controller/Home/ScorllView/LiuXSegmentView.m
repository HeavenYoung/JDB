...
-(instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titleArray clickBlick:(btnClickBlock)block{
    ...
    if (self) {
        ...
        if (titleArray.count<MAX_TitleNumInWindow+1) {
            _btn_w=MAIN_SCREEN_WIDTH/titleArray.count;
        }else{
            _btn_w=MAIN_SCREEN_WIDTH/MAX_TitleNumInWindow;
        }
        _titles=titleArray;
        _defaultIndex=1;
        ...
        _titleSelectColor=RGB(242, 179, 32);
        _bgScrollView=[[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, MAIN_SCREEN_WIDTH, self.frame.size.height)];
...
-(void)btnClick:(UIButton *)btn{
    ...
    CGFloat maxOffsetX= _bgScrollView.contentSize.width-MAIN_SCREEN_WIDTH;
